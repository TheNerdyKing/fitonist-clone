  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      mm.add("(min-width: 768px)", () => {
        ...
        cards.forEach((card, index) => {
          if (index === 0) {
            ...
            gsap.set(card, { y: "120%", rotation: 4, opacity: 0 });
          }
      ...
      mm.add("(max-width: 767px)", () => {
        // Mobile: No pinning, just simple layout or basic scroll trigger
        gsap.set(".channel-card", { y: 0, rotation: 0, opacity: 1, position: "relative", marginBottom: "2rem" });
        gsap.set(".channel-card-container", { position: "relative", display: "flex", flexDirection: "column" });
      });
      ...
      <div className="h-screen flex">
          ...
          <div className="flex items-center gap-4 mb-8">
            {channels.map((channel, index) => {
              ...
              return (
                <div
                  key={channel.id}
                  className={`relative w-12 h-12 md:w-14 md:h-14 rounded-full flex items-center justify-center transition-all duration-500 ${activeIndex === index
                    ? `${channel.highlightColor} scale-110`
                    : "bg-gray-100"
                    }`}
                >
                  <Icon
                    className={`w-5 h-5 md:w-6 md:h-6 transition-colors duration-300 ${activeIndex === index ? "text-black" : "text-gray-400"
                      }`}
                  />
                </div>
              );
        ...
        <div className="hidden md:block w-1/2 h-full relative overflow-hidden">
          <div
            ...
            className="absolute inset-0 flex items-center justify-center channel-card-container"
          >
            {channels.map((channel, index) => (
              <div
                key={channel.id}
        ...
        {/* Mobile View Mobile specific list */}
        <div className="md:hidden w-full px-6 py-12 space-y-8 overflow-y-auto">
          {channels.map((channel) => (
            <div key={channel.id} className="channel-card-mobile p-8 rounded-3xl bg-gray-50 border border-black/5">
              <div className={`w-12 h-12 rounded-2xl mb-6 flex items-center justify-center ${channel.color}`}>
                <channel.icon className="w-6 h-6 text-black" />
              </div>
              <h3 className="text-2xl font-bold text-black mb-4">{channel.title}</h3>
